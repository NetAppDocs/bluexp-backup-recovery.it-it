...
[NOTE]
====
Se stai creando backup nelle aree di AWS GovCloud o AWS China, dovrai modificare il nome risorsa AWS "arn" in tutte le sezioni _Resource_ dei due set di autorizzazioni seguenti:
...
Se crei i tuoi bucket, dovresti utilizzare un nome bucket che inizia con "netapp-backup". Se è necessario utilizzare un nome personalizzato, modificare i criteri utilizzati nella sezione delle autorizzazioni di impostazione S3 per utilizzare un codice ARN di risorsa che corrisponda al proprio prefisso bucket personalizzato.
...
[NOTE]
====
Se i bucket personalizzati si trovano nelle regioni di AWS GovCloud o AWS China, dovrai modificare il nome risorsa AWS "arn" in tutte le sezioni _Resource_ delle autorizzazioni:

* Per le regioni di AWS GovCloud, ad esempio, è necessario modificare "aws" in "aws-us-gov" `arn:aws-us-gov:s3:::*`.
* Per le regioni AWS China, ad esempio, è necessario modificare "aws" in "aws-cn" `arn:aws-cn:s3:::-*`.
