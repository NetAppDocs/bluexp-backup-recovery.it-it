...
[role="quick-margin-para"]
In alternativa, è possibile impostare chiavi personalizzate per la crittografia dei dati invece di utilizzare le chiavi di crittografia predefinite di Google Cloud. <<Prepare Google Cloud as your backup target,Scopri come preparare il tuo ambiente Google Cloud per ricevere i backup di ONTAP.>>
...
** *Criterio di backup*: Selezionare un criterio di archiviazione di Backup in oggetto esistente.
